...
> 本文档是分工1 交付数据的清单（dataset manifest），供下游自检字段与口径是否对齐。
> 在原有 5 张 csv 基础上，新增 3 张表、增强 1 张表，用于支撑三项分析：出行时长、线路客流强度、核心城区 vs 郊区差异。
...
> 注：生成脚本 `build_supplementary_tables.py` 与清洗脚本一样**暂不入库**（数据已交付，见 `任务清单/分工1_数据工程.md`）；如需复现或重算阈值，可向分工1 索取。
