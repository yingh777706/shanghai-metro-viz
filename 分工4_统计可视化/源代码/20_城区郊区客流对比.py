...
ax = axes[0]
b = ax.bar(x, ring_avg.values / 1e4, 0.55,
           color=[C_MAIN, C_ACCENT, C_HBO, C_NHB])
for bb, v in zip(b, ring_avg.values):
    ax.text(bb.get_x() + bb.get_width()/2, v / 1e4, f"{v/1e4:.2f}万", ha="center", fontsize=10)
...
for xi, n in zip(x, nst.loc[ring_order]):
    ax.text(xi, ax.get_ylim()[1]*0.02, f"{n}站", ha="center", fontsize=9, color="white",
            fontweight="bold")

ax = axes[1]
b = ax.bar(x, tide["in_share"].values, 0.55, color=[C_MAIN, C_ACCENT, C_HBO, C_NHB])
...
fig.suptitle("城区 / 郊区客流对比: 规模与潮汐", fontsize=15, fontweight="bold")
