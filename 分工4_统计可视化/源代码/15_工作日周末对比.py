...
fig, axes = plt.subplots(2, 2, figsize=(9.75, 6.75))
labels = ["工作日", "周末"]; x = np.arange(2); cols = [C_MAIN, C_ACCENT]

ax = axes[0, 0]
vals = np.array([daily.loc[1], daily.loc[0]]) / 1e4
b = ax.bar(x, vals, 0.28, color=cols)
for bb in b: ax.text(bb.get_x()+0.14, bb.get_height(), f"{bb.get_height():.0f}万", ha="center")
ax.set_xticks(x); ax.set_xticklabels(labels); ax.set_title("日均全网客流量")
ax.set_ylabel("万人次")

ax = axes[0, 1]
b = ax.bar(x, [peak_share[1], peak_share[0]], 0.28, color=cols)
for bb in b: ax.text(bb.get_x()+0.14, bb.get_height(), f"{bb.get_height():.1f}%", ha="center")
ax.set_xticks(x); ax.set_xticklabels(labels)
ax.set_title("早晚高峰进站客流占比"); ax.set_ylabel("%")

ax = axes[1, 0]
b = ax.bar(x, [c_share[1], c_share[0]], 0.28, color=cols)
for bb in b: ax.text(bb.get_x()+0.14, bb.get_height(), f"{bb.get_height():.1f}%", ha="center")
ax.set_xticks(x); ax.set_xticklabels(labels)
ax.set_title("通勤出行(C)占进站客流比例"); ax.set_ylabel("%")

ax = axes[1, 1]
y = np.arange(len(st10)); h = 0.24
ax.barh(y + h/2, st10[1] / 1e4, h, color=C_MAIN, label="工作日日均")
ax.barh(y - h/2, st10[0] / 1e4, h, color=C_ACCENT, label="周末日均")
ax.set_yticks(y); ax.set_yticklabels(st10.index, fontsize=11)
ax.set_xlabel("万人次"); ax.set_title("TOP10站点 工作日/周末日均客流")
ax.legend()

fig.suptitle("工作日 vs 周末 多维对比", fontsize=17, fontweight="bold")
fig.tight_layout(rect=[0, 0.02, 1, 0.96])
fig.text(0.99, 0.005, "数据来源: MetroFlow 刷卡数据集 | 分工4", ha="right", fontsize=10, color=C_GRAY)
