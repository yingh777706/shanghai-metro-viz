...
def data_path(name):
    """按文件名在数据目录中查找,返回完整路径"""
    return _find_file([name], _data_dirs(project_root()))

def load_od_travel_time():
    """OD出行时长/距离估计表(分工1补充数据)"""
    return pd.read_csv(data_path("od_travel_time.csv"))

def load_line_map():
    """站点-线路对照表(分工1补充数据)"""
    return pd.read_csv(data_path("station_line_map.csv"))

def load_area_class():
    """站点区位分类表(分工1补充数据)"""
    return pd.read_csv(data_path("station_area_class.csv"))

def load_weather():
    """逐小时天气表"""
    return pd.read_csv(data_path("weather_hourly.csv"))

def load_calendar_df():
    """工作日日历(含weekday列)"""
    cal = pd.read_csv(data_path("workday_calendar.csv"))
    cal.columns = [c.strip() for c in cal.columns]
    cal = cal.rename(columns={"isWorday": "isWorkday"})
    cal["date"] = cal["date"].astype(str)
    return cal
