...
ax.text(8, ax.get_ylim()[1]*0.97, "早高峰", ha="center", color=C_C, fontsize=13)
ax.text(18, ax.get_ylim()[1]*0.97, "晚高峰", ha="center", color=C_HBO, fontsize=13)
ax.set_title("上海地铁分时段客流曲线(10分钟粒度,日均)")
ax.set_xlabel("时刻"); ax.set_ylabel("客流量(万人次/10分钟)")
ax.set_xticks(range(6, 24))
ax.legend()
fig.text(0.99, 0.01, "数据来源: MetroFlow 刷卡数据集 | 分工4", ha="right", fontsize=10, color=C_GRAY)
